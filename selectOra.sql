...
select 
    'ID_TRANSAKSI|ID_FRANCHISE|FRANCHISE|TANGGAL_TRANSAKSI|ID_PRODUK|NAMA_PRODUK|JUMLAH_TERJUAL|STOCK|DISCOUNT|PPN|PPH4|PPH23|STATUS_PEMBAYARAN|TANGGAL_PEMBAYARAN' 
from dual;
select 
    id_transaksi || '|' || id_FRANCHISE || '|' || FRANCHISE || '|' || tanggal_transaksi || '|' || id_produk || '|' || nama_produk || '|' || jumlah_terjual || '|' || stock || '|' || discount || '|' || PPN || '|' || PPH4 || '|' || PPH23 || '|' || status_pembayaran || '|' || tanggal_pembayaran 
from
